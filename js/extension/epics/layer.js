...
// Helper function to build feature info request for a layer
const buildFeatureInfoRequest = (layerItem, store, point) => {
    const env = localizedLayerStylesEnvSelector(store.getState());
    const layers = getPluginCfg(store.getState()).layersCfg;
    const tocLayer = layersSelector(store.getState()).filter(lyr => lyr.name === get(layers, layerItem.name).nom)[0];

    const {url, request, metadata} = buildIdentifyRequest(tocLayer, {
...
// Helper function to extract features from element array
const extractFeaturesFromElements = (elementArray) => {
    const toObject = Object.assign({}, ...elementArray);
    const layerNames = elementArray.map(m => keys(m)[0]);
    const layerFeaturesArray = layerNames.map(r => ({[r]: toObject[r].data.features}));
    const features = Object.assign({}, ...layerFeaturesArray);
    return {toObject, features};
};

// Helper function to get visible layers list
const getVisibleLayersList = (store, point) => {
    const layers = getPluginCfg(store.getState()).layersCfg;
    return keys(layers).map(l => ({
        name: l,
...
export const onTabouMapClick = (action$, store) =>
    action$.ofType(CLICK_ON_MAP)
        .filter(() => isTabou2Activate(store.getState()))
        .switchMap(({point}) => {
            const list = getVisibleLayersList(store, point);

            return Rx.Observable.concat(
                Rx.Observable.of(cleanTabouSelection()),
                Rx.Observable.from(list)
                    .map(r => buildFeatureInfoRequest(r, store, point))
                    .toArray()
                    .switchMap((requestArray) =>
                        Rx.Observable.forkJoin(requestArray).flatMap(elementArray => {
                            const {toObject, features} = extractFeaturesFromElements(elementArray);
                            return Rx.Observable.of(
